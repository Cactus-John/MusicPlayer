...
function loadTrack(track_index) {
    ...
    curr_track.src = music_list_hob[track_index].music;
    curr_track.load();

    track_art.style.backgroundImage = "url(" + music_list_hob[track_index].img + ")";
    track_name.textContent = music_list_hob[track_index].name;
    track_artist.textContent = music_list_hob[track_index].artist;
    now_playing.textContent = "Playing music " + (track_index + 1) + " of " + music_list_hob.length;
...
function nextTrack(){
    if(track_index < music_list_hob.length - 1 && isRandom === false) {
        track_index += 1;
    }
    else if(track_index < music_list_hob.length - 1 && isRandom === true) {
        let random_index = Number.parseInt(Math.random() * music_list_hob.length);
...
function prevTrack() {
    if(track_index > 0) {
        ...
        track_index = music_list_hob.length -1;
    }
    loadTrack(track_index);
    playTrack();
}
